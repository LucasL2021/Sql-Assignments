...
select concat(p.Name, p.Color) from Production.Product p where p.Color is not null


--7.Write a query that generates the following result set  from Production.Product:
select concat('NAME: ', p.Name,'  --  COLOR: ', p.Color) from Production.Product p where p.Color is not null
